...
async def get_topic_with_channels(db: AsyncSession, topic_slug: str) -> Topic:
    ...
    if not topic:
        raise HTTPException(
            ...
            detail=f"Topic '{topic_slug}' no encontrado",
        )
    return topic
...
async def add_channel(
    ...
    if existing.scalar_one_or_none():
        raise HTTPException(
            ...
            detail=f"Canal con URL '{url}' ya existe en topic '{topic_slug}'",
        )
...
async def delete_channel(
    ...
    if not channel:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Canal '{channel_name}' no encontrado en topic '{topic_slug}'",
        )
    ...
    if channel_count <= 1:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="No se puede eliminar el unico canal del topic",
        )
